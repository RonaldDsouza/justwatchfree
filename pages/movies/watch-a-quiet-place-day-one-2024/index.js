...
const moviesDetail = ({ movie }) => {
  ...
  const enhancedParagraph = text => {
    const linkTargets = [
      {
        text: 'A Quiet Place Day One - 2024',
  ...
  const languagesSchema = JSON.stringify({
    '@context': 'https://schema.org',
    '@type': 'WebPage',
    url: 'https://justwatchfree.vercel.app/movies/watch-a-quiet-place-day-one-2024',
    name: 'Watch Movie A Quiet Place Day One (2024) | JWF™',
    alternateName: [
      'Ver película Un lugar tranquilo Día uno (2024) | JWF™',
      'Regarder le film Un endroit calme Jour un (2024) | JWF™',
      'Film Ein ruhiger Ort Tag eins (2024) ansehen | JWF™',
      '观看电影 寂静之地第一天 (2024) | JWF™',
      '映画 静かな場所 デイワン (2024) を見る | JWF™',
      '영화 조용한 곳 첫날 (2024) 보기 | JWF™',
      'Assistir Filme Um Lugar Silencioso Dia Um (2024) | JWF™',
      'Guarda il film Un posto tranquillo Giorno uno (2024) | JWF™',
      'Посмотреть фильм Тихое место День первый (2024) | JWF™',
      'مشاهدة فيلم مكان هادئ اليوم الأول (2024) | جي دبليو إف™'
    ],
    inLanguage: [
  ...
  return (
    <div>
      <Head>
        ...
        <title>Watch Movie A Quiet Place Day One (2024) | JWF™</title>
...
export async function getServerSideProps () {
  ...
  const selectedMovie = data.find(movie => movie.id === 'INDEX48')
  return {
    props: {
      movie: selectedMovie
    }
