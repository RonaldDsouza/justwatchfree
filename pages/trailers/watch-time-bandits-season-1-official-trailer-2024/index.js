...
const trailersDetail = ({ trailers }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('https://justwatchfree.vercel.app/trailers.json')
    ...
    const linkTargets = [
      {
        ...
        url: 'https://www.imdb.com/title/tt1928307/'
      }
    ]
  ...
  const rankMathSchema = JSON.stringify({
    ...
    '@graph': [
      ...
      },
      {
        ...
        headline: ` ${trailers.name} | JWF™`,
        datePublished: trailers.datePublished,
        dateModified: trailers.dateModified,
        articleSection: 'Movies & TV Series ',
        author: {
          '@id': 'https://justwatchfree.vercel.app/author/justwatchfree/'
        },
        publisher: {
  ...
  return (
    <div>
      <Head>
        ...
        />
        <title>Watch Time Bandits Season 1 Official Trailer | JWF™</title>
        ...
        <meta
          property='og:title'
          content={`${trailers && trailers.name} - JWF™`}
        />
        <meta
        ...
        <meta name='twitter:card' content='summary_large_image' />
          ...
          <meta
            name='twitter:image'
            content={`${trailers && trailers.image1}`}
          />
...
export async function getServerSideProps () {
  const res = await fetch('https://justwatchfree.vercel.app/trailers.json')
  const data = await res.json()
  const selectedTrailers = data.find(trailers => trailers.id === 'INDEX10')
  return {
    props: {
