...
const moviesDetail = ({ movie }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('https://justwatchfree.vercel.app/movies.json')
  ...
  const enhancedParagraph = text => {
    const linkTargets = [
      {
        text: 'Rebel Moon - Part One: A Child of Fire - 2024',
  ...
  const rankMathSchema = JSON.stringify({
    ...
    '@graph': [
      ...
      },
      {
        ...
        sameAs: ['https://justwatchfree.vercel.app']
      },
      {
        '@type': 'Article',
        '@id': `${movie.siteurl}#article`,
        headline: ` ${movie.name} | JWF™`,
        datePublished: movie.datePublished,
        dateModified: movie.dateModified,
  ...
  const languagesSchema = JSON.stringify({
    '@context': 'https://schema.org',
    '@type': 'WebPage',
    url: 'https://justwatchfree.vercel.app/movies/watch-rebel-moon-part-one-a-child-of-fire-2024',
    name: 'Watch Movie Rebel Moon - Part One: A Child of Fire (2024) | JWF™',
    alternateName: [
      'Ver película Rebel Moon - Part One: A Child of Fire (2024) | JWF™',
      'Regarder le film Rebel Moon - Part One: A Child of Fire (2024) | JWF™',
      'Film Rebel Moon - Part One: A Child of Fire (2024) ansehen | JWF™',
      '观看电影 Rebel Moon - Part One: A Child of Fire (2024) | JWF™',
      '映画 Rebel Moon - Part One: A Child of Fire (2024) を見る | JWF™',
      '영화 Rebel Moon - Part One: A Child of Fire (2024) 보기 | JWF™',
      'Assistir Filme Rebel Moon - Part One: A Child of Fire (2024) | JWF™',
      'Guarda il film Rebel Moon - Part One: A Child of Fire (2024) | JWF™',
      'Посмотреть фильм Rebel Moon - Part One: A Child of Fire (2024) | JWF™',
      'مشاهدة فيلم Rebel Moon - Part One: A Child of Fire (2024) | جي دبليو إف™'
    ]
    ,
    inLanguage: [
  ...
  return (
    <div>
      <Head>
        ...
        <title>Watch Movie Rebel Moon - Part One: A Child of Fire (2024) | JWF™</title>
        ...
        <meta property='og:title' content={`${movie && movie.name} - JWF™`} />
        <meta
...
export async function getServerSideProps () {
  const res = await fetch('https://justwatchfree.vercel.app/movies.json')
  const data = await res.json()
  const selectedMovie = data.find(movie => movie.id === 'INDEX54')
  return {
    props: {
      movie: selectedMovie
    }
