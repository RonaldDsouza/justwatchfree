...
const adultDetail = ({ adult }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('https://justwatchfree.vercel.app/adult.json')
      const data = await response.json()

      // Get 5 random items
      const randomAdultData = getRandomItems(data, 7)
  ...
  const rankMathSchema = JSON.stringify({
    ...
    '@graph': [
      ...
      },
      {
        ...
        url: adult.siteurl,
        name: `${adult.name} |  Just Watch Free™`,
      ...
      {
        '@type': 'Person',
        '@id': 'https://justwatchfree.vercel.app/author/azmovies/',
        name: 'Dr Trailer',
        url: 'https://justwatchfree.vercel.app/author/azmovies/',
        image: {
          '@type': 'ImageObject',
      ...
      },
      {
        ...
        headline: ` ${adult.name} |  Just Watch Free™`,
        datePublished: adult.datePublished,
        dateModified: adult.dateModified,
  ...
  const languagesSchema = JSON.stringify({
    '@context': 'https://schema.org',
    '@type': 'WebPage',
    url: 'https://justwatchfree.vercel.app/adult/watch-painter-babu-part-01-2024',
    name: 'Watch Adult Painter Babu Part 01 (2024) | JWF™',
    alternateName: [
      'Ver Painter Babu Parte 01 (2024) para adultos | JWF™',
      'Regarder Painter Babu Partie 01 (2024) pour adultes | JWF™',
      'Painter Babu Teil 01 (2024) für Erwachsene ansehen | JWF™',
      '观看 Painter Babu 第01部分 (2024) 成人内容 | JWF™',
      'Painter Babu パート01 (2024) 大人向け | JWF™',
      'Painter Babu 파트 01 (2024) 성인 전용 | JWF™',
      'Assistir Painter Babu Parte 01 (2024) para adultos | JWF™',
      'Guarda Painter Babu Parte 01 (2024) per adulti | JWF™',
      'Посмотреть Painter Babu Часть 01 (2024) для взрослых | JWF™',
      'مشاهدة Painter Babu الجزء 01 (2024) للكبار فقط | JWF™'
    ],
    inLanguage: [
  ...
  return (
    <div>
      <Head>
        ...
        />
        <title> Watch Painter Babu Part 01 (2024) | JWF™</title>
        ...
        <meta
          property='og:title'
          content={`${adult && adult.name} -  Just Watch Free`}
        />
        <meta
...
export async function getServerSideProps () {
  const res = await fetch('https://justwatchfree.vercel.app/adult.json')
  const data = await res.json()
  const selectedAdult = data.find(adult => adult.id === 'INDEX14')
  return {
    props: {
      adult: selectedAdult
    }
